export const errorCodes = {
  AUTH_INVALID_TOKEN: "AUTH_INVALID_TOKEN",
  AUTH_EXPIRED_TOKEN: "AUTH_EXPIRED_TOKEN",
  AUTH_MISSING_TOKEN: "AUTH_MISSING_TOKEN",
  AUTH_INACTIVE_TOKEN: "AUTH_INACTIVE_TOKEN",
  AUTH_UNAUTHORIZED: "AUTH_UNAUTHORIZED",
  AUTH_FORBIDDEN: "AUTH_FORBIDDEN",
  AUTH_INVALID_CREDENTIALS: "AUTH_INVALID_CREDENTIALS",
  AUTH_PASSWORD_REUSE: "AUTH_PASSWORD_REUSE",
  AUTH_USER_NOT_VERIFIED: "AUTH_USER_NOT_VERIFIED",
  VALIDATION_SCHEMA_FAILED: "VALIDATION_SCHEMA_FAILED",
  VALIDATION_PAYLOAD_INVALID: "VALIDATION_PAYLOAD_INVALID",
  INVALID_UPDATE_PAYLOAD: "INVALID_UPDATE_PAYLOAD",
  USER_CONFLICT: "USER_CONFLICT",
  USER_NOT_FOUND: "USER_NOT_FOUND",
  USER_CREATE_FAILED: "USER_CREATE_FAILED",
  USER_RETRIEVE_FAILED: "USER_RETRIEVE_FAILED",
  USER_UPDATE_FAILED: "USER_UPDATE_FAILED",
  EMAIL_SEND_FAILED: "EMAIL_SEND_FAILED",
  SYSTEM_UNEXPECTED: "SYSTEM_UNEXPECTED",
  SYSTEM_TOKEN_VALIDATION_FAILED: "SYSTEM_TOKEN_VALIDATION_FAILED",
  RESOURCE_NOT_FOUND: "RESOURCE_NOT_FOUND",
} as const;

export const errorLevel: Record<ErrorCode, "warn" | "error"> = {
  AUTH_INVALID_TOKEN: "warn",
  AUTH_EXPIRED_TOKEN: "warn",
  AUTH_MISSING_TOKEN: "warn",
  AUTH_INACTIVE_TOKEN: "warn",
  AUTH_UNAUTHORIZED: "warn",
  AUTH_FORBIDDEN: "warn",
  AUTH_INVALID_CREDENTIALS: "warn",
  AUTH_PASSWORD_REUSE: "warn",
  AUTH_USER_NOT_VERIFIED: "warn",
  VALIDATION_SCHEMA_FAILED: "warn",
  VALIDATION_PAYLOAD_INVALID: "warn",
  USER_CONFLICT: "warn",
  USER_NOT_FOUND: "warn",
  RESOURCE_NOT_FOUND: "warn",
  INVALID_UPDATE_PAYLOAD: "error",
  USER_CREATE_FAILED: "error",
  USER_RETRIEVE_FAILED: "error",
  USER_UPDATE_FAILED: "error",
  EMAIL_SEND_FAILED: "error",
  SYSTEM_UNEXPECTED: "error",
  SYSTEM_TOKEN_VALIDATION_FAILED: "error",
};

export const internalOnlyErrorCodes = new Set<ErrorCode>([
  "INVALID_UPDATE_PAYLOAD",
  "USER_CREATE_FAILED",
  "USER_RETRIEVE_FAILED",
  "USER_UPDATE_FAILED",
  "EMAIL_SEND_FAILED",
  "SYSTEM_TOKEN_VALIDATION_FAILED",
]);

export const conditionalErrorCodes = new Set<ErrorCode>(["USER_NOT_FOUND"]);

export const userFacingErrorCodes = new Set<ErrorCode>([
  "AUTH_INVALID_TOKEN",
  "AUTH_EXPIRED_TOKEN",
  "AUTH_MISSING_TOKEN",
  "AUTH_INACTIVE_TOKEN",
  "AUTH_UNAUTHORIZED",
  "AUTH_FORBIDDEN",
  "AUTH_PASSWORD_REUSE",
  "AUTH_INVALID_CREDENTIALS",
  "AUTH_USER_NOT_VERIFIED",
  "VALIDATION_SCHEMA_FAILED",
  "VALIDATION_PAYLOAD_INVALID",
  "USER_CONFLICT",
  "RESOURCE_NOT_FOUND",
]);

export type ErrorCode = (typeof errorCodes)[keyof typeof errorCodes];
