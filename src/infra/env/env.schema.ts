...
export const envSchema = z
  .object({
    ...
    APP_PORT: z
      .string({
        required_error: "APP_PORT is required",
        invalid_type_error: "APP_PORT must be a string",
      })
      .refine(
        (port) => {
    ...
    FRONTEND_ORIGIN: z.string({
      ...
      invalid_type_error: "FRONTEND_ORIGIN must be a string",
    }),
  ...
  .superRefine((env, ctx) => {
    try {
      const originUrl = new URL(env.APP_ORIGIN);
      const originPort =
        originUrl.port || (originUrl.protocol === "https:" ? "443" : "80");

      if (originPort !== env.APP_PORT) {
        ctx.addIssue({
          path: ["APP_ORIGIN"],
          message: "APP_ORIGIN port must match APP_PORT",
